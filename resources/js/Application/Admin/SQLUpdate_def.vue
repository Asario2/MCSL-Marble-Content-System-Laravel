...
export default {
  ...
  methods: {
    ...
    try {
        ...
        this.diffName = res.data.name || [];
        console.log(this.diffName);
   ...
   async syncToAll() {
  this.loading = true;
  this.syncFinished = false;   // 👈 wichtig
  this.syncStatus = "";        // optional leeren
  ...
  try {
    ...
    // ✅ ERST HIER Status setzen
    this.syncStatus = `
      <strong>SyncToAll abgeschlossen!</strong><br>
      <span class='text-blue-400 font-bold'>Local → Online:</span> ${localToOnline.length} Tabellen<br>
      <span class='text-purple-700 font-bold'>Online → Local:</span> ${onlineToLocal.length} Tabellen
    `;
